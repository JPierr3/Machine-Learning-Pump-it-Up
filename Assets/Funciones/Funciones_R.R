...
sensEspCorte2<-function(modelo,dd,nombreVar,ptoCorte,evento){
  probs <-predict(modelo,newdata=dd,type="response")
  cm<-confusionMatrix(data=factor(ifelse(probs>ptoCorte,1,0)), reference=as.factor(dd[,nombreVar]),positive=evento)
  c(cm$overall[1],cm$byClass[1:4])
} 
